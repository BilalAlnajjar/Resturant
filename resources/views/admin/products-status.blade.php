...
<div class="card">
    <form class=" p-4 row" style="min-width: 100px;" method="POST" action="{{route('product_status')}}" enctype="multipart/form-data">
        @csrf
            <select class="col-4 form-control select2 custom-select" data-placeholder="Choose one" name="category_id">
                @foreach ($categories as $item)
                    <option @if($category->id == $item->id) selected @endif value="{{$item->id}}" >{{$item->name}}</option>
                @endforeach
            </select>
        ...
        @if($products->count() != 0)
            @foreach($products as $item)
                <li id="item_{{$item->id}}" class="list-group-item">
                    <img src="{{asset($item->image)}}" style="width: 20px;height: 20px;border-radius: 100%;">
                    {{$item->name}}
